    ...
    <div className={`layout ${mode}`.trim()}>
      ...
      <main className="main-shell">
        ...
        {activeTab === "timeline" && mode !== "desktop" ? (
          <IconButton className="fab" aria-label="Nuevo Post" onClick={openCreatePostComposer}>
      ...
      {showAuthPrompt ? (
        <SheetOverlay className="auth-prompt-overlay" role="dialog" aria-modal="true" aria-label="Acceso" onClick={() => setShowAuthPrompt(false)}>
          <SheetCard className="auth-prompt-card" onClick={(event) => event.stopPropagation()}>
            <IconButton className="auth-prompt-close" aria-label="Cerrar" onClick={() => setShowAuthPrompt(false)}>
